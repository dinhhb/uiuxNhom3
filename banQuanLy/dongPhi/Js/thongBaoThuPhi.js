var activeItems = tableDonTamVang.filter(item => item.tinhTrang === "Chưa đóng");
...
function buildTable() {
    ...
    var startIndex = myList.findIndex(item => item.tinhTrang === "Chưa đóng");
    ...
    for (var i = startIndex; i < myList.length; i++) {
        if (myList[i].tinhTrang === "Chưa đóng") {
            ...
            // Only proceed if the tinhTrang is "Chưa đóng"
            var row = `<tr>
                        <td><input type="checkbox" class="selectItem" value="${myList[i].id}"></td> <!-- Checkbox cho mỗi hàng -->
                        <td>${stt}</td>
...
var selectedIds = []; // Mảng để lưu trữ ID của các hàng được chọn
...
document.getElementById('sendNoti').addEventListener('click', function (event) {
    event.preventDefault()
    console.log("popup");
    updateModalContent('#confirmationModal', 'Bạn có chắc chắn muốn gửi thông báo tới những chủ hộ này không?');
    $('#confirmationModal').modal('show');
    ...
    document.getElementById('btn-xac-nhan').addEventListener('click', function () {
        updateModalContent('#successModal', 'Thông báo đã được gửi đi thành công');
